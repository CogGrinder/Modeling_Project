...
class Utils_starter_5:
    ...
    def get_pix_at_translated(self, x : int, y : int, p : list):
        ...
        if 1<=x-int_px<n and 1<=y-int_py<m:
            ...
            return    (1-float_px)*( (1-float_py) *self._img2.data[x-int_px][y-int_py] \
                                    + float_py    *self._img2.data[x -int_px][y-int_py-1]) \
                    + float_px   * ( (1-float_py) *self._img2.data[x-int_px-1][y-int_py] \
                                    + float_py    *self._img2.data[x -int_px-1][y-int_py-1]) #TODO probably bugs here with indexation
        else:
        ...
        float_px = p[0] - int_px
        float_py = p[1] - int_py
        #optimised conditional
        condition_matrix = np.logical_and(np.logical_and(1+int_px<=x, x<n+int_px) ,
                                        np.logical_and(1+int_py<=y, y<m+int_py))
        # condition_matrix = np.zeros((n,m)).astype(bool)
        # condition_matrix[n//2,m//2] = True

        xcondition = x.copy()
        ycondition = y.copy()
        xcondition[np.logical_not(condition_matrix)] = int_px +1 # used to nullify the index in the np.where below
        ycondition[np.logical_not(condition_matrix)] = int_py +1

        return np.where(condition_matrix,
                        
                        (1-float_px)*( (1-float_py) *self._img2.data[xcondition-int_px,   ycondition-int_py] \
                                        + float_py  *self._img2.data[xcondition -int_px,  ycondition-int_py-1]) \
                        + float_px  *( (1-float_py) *self._img2.data[xcondition-int_px-1, ycondition-int_py] \
                                        + float_py  *self._img2.data[xcondition -int_px-1,ycondition-int_py-1]),
                         1,)
        
        #1 is white padding TODO check this condition when calculating loss
    ...
    def loss_function_1(self,**kwargs):#(self, p : list, warp : callable = get_pix_at_translated): #[[int,int,list],float]
        ...
        i,j = np.meshgrid(np.arange(self._img2.data.shape[0]), np.arange(self._img2.data.shape[1]),indexing="ij")
        
        warped_img2 = warp(i,j,p) # uses numpy arrays here
    ...
    def make_save_name(self, loss_function:callable) :
        return  self._img1.name + "_" + self._img2.name + "_" + str(loss_function.__name__) + ".txt"

    def import_plot(self, loss_function:callable) :
        """Imports loss_function data from named .txt file

        Args:
            loss_function (callable): loss function used for finding the name of the file
        ...
        if os.path.exists(save_filename) :
            ...
            while(not compute in ["y","n"]):
                ...
                compute = input(f"No data for {loss_function.__name__}. Do you want to compute ? (y/n)")
            if compute == "y":
                return self.plot_loss(show=False,span="all")
            else:
    ...
    def plot_loss(self, **kwargs) : #TODO : vary the ranges and shapes for p, maybe with relation to loss function and image data
        ...
        if show :
            ...
            if compute == "y":
                ...
                px, py, loss_grid = self.import_plot(loss_function)
                ax.plot_surface(px,py,loss_grid)
